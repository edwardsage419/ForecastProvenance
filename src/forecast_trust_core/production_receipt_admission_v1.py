...
def validate_production_receipt_profile_binding(
    ...
    *,
    ...
    if not verify_sealed_object(provider_profile):
        return aggregate(
            [_check("production_receipt.profile_seal", False, "INVALID_PROVIDER_PROFILE_SEAL")]
        )
    if not verify_sealed_object(qualification_decision):
        return aggregate(
            [_check("production_receipt.decision_seal", False, "INVALID_QUALIFICATION_DECISION_SEAL")]
        )
        ...
        return aggregate(
            [
                _check(
                    ...
                    False,
                    "INVALID_QUALIFICATION_STATE_PACKAGE_SEAL",
                )
            ]
        )
