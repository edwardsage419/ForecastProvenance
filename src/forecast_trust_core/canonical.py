...
def timestamp_le(left: str, right: str) -> bool:
    require_utc_timestamp(left)
    require_utc_timestamp(right)
    return left <= right
...
def _validate_tree(value: Any, path: str = "$") -> None:
    ...
    if isinstance(value, list):
        for idx, member in enumerate(value):
            _validate_tree(member, f"{path}[{idx}]")
        return
    if isinstance(value, dict):
        for key, member in value.items():
            if not isinstance(key, str) or not key.isascii():
                raise CanonicalizationError(f"object keys must be ASCII at {path}")
...
def validate_ref(ref: Mapping[str, Any]) -> None:
    if set(ref) != {"object_id", "content_sha256"}:
        raise CanonicalizationError("reference must contain object_id and content_sha256 only")
...
def seal_object(payload: Mapping[str, Any], *, object_type: str, stable_context: str, semantic_id: str | None = None) -> dict[str, Any]:
    require_ascii_token(object_type, "object_type")
    require_ascii_token(stable_context, "stable_context")
    forbidden = {"object_id", "payload_sha256", "content_sha256"} & set(payload)
    if forbidden:
        raise CanonicalizationError(f"payload contains derived fields: {sorted(forbidden)}")
    base = dict(payload)
    ...
    final_without_content = dict(base)
    final_without_content["object_id"] = object_id
    final_without_content["payload_sha256"] = payload_sha
    final = dict(final_without_content)
    final["content_sha256"] = content_hash(final_without_content)
    return final
...
def verify_sealed_object(obj: Mapping[str, Any]) -> bool:
    ...
    if not required.issubset(obj):
        return False
    try:
        require_ascii_token(obj["object_type"], "object_type")
        require_ascii_token(obj["object_id"], "object_id")
        without_content = {k: v for k, v in obj.items() if k != "content_sha256"}
        expected_content = content_hash(without_content)
        payload = {k: v for k, v in obj.items() if k not in {"object_id", "payload_sha256", "content_sha256"}}
        expected_payload = content_hash(payload)
